module PaperTrail
  module Model
    ...
    module InstanceMethods
      ...
      def record_create
        if switched_on?
          v = versions.new merge_metadata(:event => 'create',	 
          								  :whodunnit => PaperTrail.whodunnit )
       	  v.attributes = { :item_type => self.class.name}
          v.save
      ...
      def record_update
        if switched_on? && changed_notably?
          v = versions.build merge_metadata(:event     => 'update',
									        :item_type => 'Audit',
      ...
      def record_destroy
        if switched_on? and not new_record?
          v = Version.create merge_metadata(:item      => self,
							             :item_type => 'Audit',
                                         :event     => 'destroy',
                                         :object    => object_to_string(item_before_change),
                                         :whodunnit => PaperTrail.whodunnit)
